# -*- coding: utf-8 -*-
"""
Created on Wed Jun 21 09:53:05 2023
...
def add_dots(lon, lat,diff,vmin,vmax):    
    ...
    for i, ax in enumerate(axgr):
        ...
        p = ax.scatter(lon, lat, c=diff, s=10, cmap=cmaps.GMT_panoply,vmin=vmin, vmax=vmax,marker = "o")
    axgr.cbar_axes[0].colorbar(p)
    #ax.set_title(title, fontsize='medium')
    plt.savefig('C:/Users/fanwang/Desktop/tw_summer_trend.png',bbox_inches = 'tight',dpi=300)
    plt.show()
...
for ff in files:
    ...
    for ss in range(826):
        ...
        for y0 in (np.unique(tmp[:,1])):
            #print(y0)
            #index2 = np.where((tmp[:,1]==y0) & ((tmp[:,2]>11) | (tmp[:,2]<3))) #winter
            #index2 = np.where((tmp[:,1]==y0))
...
for i in range(826):
    ...
    mois[i] = stats.linregress(np.arange(40), year_ave[i,19::])[0]
    mois_p[i] = stats.linregress(np.arange(40), year_ave[i,19::])[3]

index_p = np.where(mois_p<0.05)
diff = mois
add_dots(ll_sel[index_p[0],5],ll_sel[index_p[0],4],diff[index_p[0]]*10,-0.4,0.4)
